...
<form class="form-horizontal" role="form" method="POST" action="<?php echo uri('admin/testimonial/edit/' . $object->getId()) ?>">
...
<script>
  $(function(){
    ...
    $(document).on('click', '#image .upload', function(){
      ...
      $.ajax({
        url: '<?php echo uri("modules/testimonial/controllers/backend/testimonial_form_field_image.php" ,false) ?>',
        type: 'POST',
        data: formData,
        cache: false,
        dataType: 'json',
    ...
    $(document).on('click', '#image .remove', function(){
      ...
      if (typeof($(this).data('uri')) !== 'undefined') {
        ...
        $.ajax({
          url: '<?php echo uri("modules/testimonial/controllers/backend/testimonial_form_field_image"."_remove.php" ,false) ?>?path=' + encodeURIComponent(img),
          type: 'POST',
          dataType: 'json',
          success: function(data, textStatus, jqXHR) {
            if (typeof(data.error) !== 'undefined') {
...
<div class='form-group'>
  ...
  <div class='col-sm-10'>
    <input value='<?php echo htmlentities(str_replace('\'', '"', ($object->isNew() ? (isset($_POST['from']) ? strip_tags($_POST['from']) : '') : $object->getFrom()))) ?>' type='text' class='form-control' id='from' name='from' />
  </div>
</div>
<div class='hr-line-dashed'></div>
